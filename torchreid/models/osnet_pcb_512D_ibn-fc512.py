...
class OSNetPCB512DIbn(nn.Module):
    ...
    def __init__(self, num_classes=1000, parts=6, loss='softmax', pretrained=False, **kwargs):
        ...
        #self.fc_parts = [self._construct_fc_layer(self.num_features, self.num_features, dropout_p=None) for _ in range(parts)]
        self.fc =  self._construct_fc_layer(self.num_features, self.num_features, dropout_p=None)
    ...
    def forward(self, x):
        ...
        # 也可以不加全连接层，看量化表现；是否要归一化  F.normalize       
        if self.fc is not None:
            averaged_features = self.fc(averaged_features)
    ...
    def _construct_fc_layer(self, fc_dims, input_dim, dropout_p=None):
        ...
        for dim in fc_dims:
            ...
            input_dim = dim
        print("[Debug]fc layers {}".format(layers))
