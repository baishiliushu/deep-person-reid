...
class OSNetPCB512DIbnAgv(nn.Module):
    """
    ...
    def __init__(self, num_classes, parts=6, pretrained=True, **kwargs):
        super(OSNetPCB512DIbnAgv, self).__init__()
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            for i in range(self.parts):
                # TODO: 对每个部分进行平均池化 -> 改为使用 AdaptiveAvgPool2d
                part = feat_map[:, :, i * part_h: (i + 1) * part_h, :]
                # TODO: 简单的全局平均 -> 改为跨部分区域的空间维度求平均或者对NPU量化更友好的平均
        ...
        #     # 训练时返回加权后的 logits
        #     # 设置权重系数
        #     weights = [1.0, 1.2, 1.2, 1.2, 1.2, 1.0]  # 中间三个部分的权重为 1.2 -> nn.Liner
        #     weighted_logits = [logit * weight for logit, weight in zip(logits, weights)]
        #     return weighted_logits  # 返回各部分的加权 logits

        #logit = self.classifier[i](x)\tprob = torch.sigmoid(logit)
        # stage I  : A. HEAD2 -> 6*sigmoid assistant 6*parts B. 512 embedding fix with image location
        # stage II : A. 6*parts change to 6*anchors B. make HEAD2 to be BRANCH2(looks like 141 classes emb-yo)
        # stage III: A. fc update by self-attention B. light weight


# 为了和其他模型一致，提供一个接口函数
def osnet_pcb_512D_Ibn_Agv(num_classes=1000, pretrained=True, **kwargs):
    model = OSNetPCB512DIbnAgv(num_classes=num_classes, parts=6, pretrained=pretrained, **kwargs)
    return model
